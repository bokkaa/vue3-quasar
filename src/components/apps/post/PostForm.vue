<template>
  <q-form >
    <q-card-section class="q-pa-md q-gutter-y-sm">
        ...
        <!-- value값만 가져오기 -->
        <q-select outlined v-model="categoryModel" :options="categories" emit-value map-options>
            <template v-if="!categoryModel" #selected>
                <span class=text-grey-7>카테고리를 선택하세요</span>
            </template>
        </q-select>
        <q-input v-model="contentModel" type="textarea" outlined placeholder="내용을 작성해주세요"/>
        <q-input v-model="tagField" outlined placeholder="태그를 입력해주세요! (입력 후 Enter)" prefix="#"/>
